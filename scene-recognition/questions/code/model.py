...
class Model:
    def __init__(self, train_images, train_labels, num_classes, hp, test_images, test_labels):
        ...
        self.b = np.zeros(shape=(1, self.num_classes))
    ...
    def forward_pass(self, inputs):
        ...
        logits = inputs @ self.W + self.b # b is an array of biases for each class
        # TODO: Get probabilities by using softmax on the logits
        def softmax(logits): # (input_size, num_classes)
            # prevent overflow
            logits -= np.max(logits)
            
            exp = np.exp(logits) # (input_size, num_classes) 
            sum_each_row = np.sum(exp) # (input_size, 1)  .. there's only one row
            prob = exp / sum_each_row
            return prob
    ...
    def loss(probabilities, gt_label):
        ...
        # print(gt_label)
        loss = -np.log(probabilities[0,gt_label] + 1e-10) # (1, # classes) * (batch, # classes) = (1, # classes)
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        gt_label_array = np.zeros(shape=probabilities.shape)
        gt_label_array[0,gt_label] = 1
        
        gradW = np.outer(img, (probabilities[0] - gt_label_array[0]))# probabilities shape = (1, # classes) I think
        gradB = probabilities[0] - gt_label_array[0]
        # TODO: Reshape train image data to be matrix, dimension [img.size, 1]
        img = img.reshape(-1, 1)
        # gradW, gradB = np.zeros(shape=probabilities.shape), np.zeros(shape=(1, probabilities.shape[1]))
        # # for each row, if argmax(row of prob) == index of gt_label where gt_label[index] = 1, then 
        # img * (probabilities - 1)
    ...
    def train_nn(self):
        ...
        Return: None
        """
        indices = list(range(self.train_images.shape[0])) # batch size

        for epoch in range(self.num_epochs):
            loss_sum = 0
            random.shuffle(indices)

            for index in range(len(indices)): # index starts from 0
                i = indices[index] # indices is shuffled. picking random image index
